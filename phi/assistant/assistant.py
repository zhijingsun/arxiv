...
class Assistant(BaseModel):
    ...
    def cli_app(
        # 定义了一个命令行应用程序 cli_app，
        # 通过 self.print_response 方法处理用户输入并生成响应。
        # 用户可以在命令行中输入消息，应用程序会生成并打印响应，直到用户输入退出命令（例如 "exit", "quit", 或 "bye"）。
        self,
    ...
    def get_response(
        ...
        **kwargs: Any,
        ...
        if stream:
            ...
            response = self.run(message=message, messages=messages, stream=False, **kwargs)  # type: ignore
            return self.convert_response_to_string(response)
